...
/**
 * Request抽象，提供如下功能: 请求信息封装 解析返回数据{@link #parseResponse(NetworkResponse response)} 分发请求数据结果
...
public abstract class Request<T> {
	...
	public static final int DEFAULT_SO_TIMEOUT = 20 * 1000; // 默认响应超时时间：20秒
	...
	protected Map<String, String> stringParams = new TreeMap<String, String>(); // HTTP
																			// 参数
	protected ArrayList<FileParameter> fileParams = null;
	protected ArrayList<ByteParameter> byteParams = null;
	...
	protected void dispatchError(HError error) {
		if (mResponseListener != null) {
			...
			if (dialog == null)
				mResponseListener.showToast = false;
	...
	public boolean containsMutilpartData() {
		return (fileParams != null && fileParams.size() > 0) || (byteParams != null && byteParams.size() > 0);
	}
	
	public void addFileParam(String paramName, String fileName, File file) {
		if (fileParams == null) {
			fileParams = new ArrayList<FileParameter>();
		}
	...
	public void addByteParam(String paramName, String fileName, byte[] data) {
		if (byteParams == null) {
			byteParams = new ArrayList<ByteParameter>();
		}
	...
	public void addParam(String name, int value) {
		this.stringParams.put(name, String.valueOf(value));
	}
	...
	public void addObjectParams(Map<String, Object> params) {
		if (params != null) {
			Set<String> keys = params.keySet();
			for (String key : keys) {
				stringParams.put(key, params.get(key).toString());
			}
		}
	}
