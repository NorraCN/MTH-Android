...
import com.norra.cloud.http.frame.Request.ByteParameter;
import com.norra.cloud.http.frame.Request.FileParameter;
...
public class HttpClientWorker implements HttpWorker {
	...
	@Override
	public HttpResponse doHttpRequest(Request<?> request) throws IOException {
	...
	private HttpUriRequest createUriRequest(Request<?> request) throws UnsupportedEncodingException {
		...
		case POST:
			...
			if (!request.containsMutilpartData()) {
				...
				if (request.getByteParams() != null && request.getByteParams().size() > 0) {
